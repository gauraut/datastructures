...
void LinkedList<T>::insertOrdered(const T& newData) {
	...
	{
		if(head_!=tail_ && newData<=tail_->data && newData>head_->data) //if head is not equal to tail
		{
			if ((newData>=0 && abs(newData-head_->data)<abs(newData-tail_->data)) || (newData<0 && abs(newData-head_->data)<abs(newData-tail_->data))) //check if newData is closer to head
			{
				Node* traverse= head_;
				while (newData>traverse->data && traverse->next)
				{
				...
				{
					opt->next=traverse;
					opt->prev=traverse->prev;
					(traverse->prev)->next=opt;
					traverse->prev=opt;
				}
			}
			else
			{
				Node* traverse= tail_;
				while(newData<=traverse->data && traverse->prev)
				{
				...
				{
					opt->prev=traverse;
					opt->next=traverse->next;
					(traverse->next)->prev=opt;
					traverse->next=opt;
				}
			}
			//size_++;
...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T>& other) const {
  ...
  // the function.
  LinkedList<T> merged;
  Node* traleft=this->head_;
  Node* traright=other.head_;
  while (merged.size()!=this->size()+other.size())
  {
	  if (left.size()!=0 && right.size()!=0 && (traleft->data<traright->data))
	  {
		 merged.pushBack(left.front());
		 left.popFront();
		 traleft=traleft->next;
	  }
	  else if (left.size()!=0 && right.size()!=0 && (traleft->data>traright->data))
	  {
		  merged.pushBack(right.front());
		  right.popFront();
		  traright=traright->next;
	  }
