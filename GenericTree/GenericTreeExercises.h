...
static void treeFactory(GenericTree<int>& tree) {
	...
	tree.createRoot(4);
	GenericTree<int>::TreeNode* NodeA= tree.getRootPtr();
	auto NodeB= NodeA->addChild(8);
	auto NodeC= NodeA->addChild(15);
	auto NodeD= NodeB->addChild(16);
	NodeB->addChild(23);
	NodeD->addChild(42);
...
std::vector<T> traverseLevels(GenericTree<T>& tree) {
  ...
  std::queue<TreeNode*> pointers;
  TreeNode* traverse = rootNodePtr;
  
  pointers.push(rootNodePtr);
  
  while (!pointers.empty())
  {
	  if(!pointers.front()->childrenPtrs.empty())
	  {
		  for (unsigned i=0; i<pointers.front()->childrenPtrs.size();i++)
		  {
			  pointers.push(pointers.front()->childrenPtrs[i]);
		  }
	  }
	  results.push_back(pointers.front()->data);
	  pointers.pop();
  }
  //      *****************************************************
  //                           EXERCISE 2
